...
vs2ps VS(vsInput input)
{
    ...
    output.LightDirV = normalize(-mul(float4(lDir,0), tV).xyz);
...
{
    return input.col * PhongDirectional(input.NormV, input.ViewDirV, input.LightDirV);
}

float4 PS_POS(vs2ps input): SV_Target
{
    return input.col_pos * PhongDirectional(input.NormV, input.ViewDirV, input.LightDirV);
}

float4 PS_GROUP(vs2ps input): SV_Target
{
    return input.col_group * PhongDirectional(input.NormV, input.ViewDirV, input.LightDirV);
}

float4 PS_GROUP_FORCE(vs2ps input): SV_Target
{
	return input.col_group_force * PhongDirectional(input.NormV, input.ViewDirV, input.LightDirV);
}

float4 PS_FORCE(vs2ps input): SV_Target
{
	return input.col_force * PhongDirectional(input.NormV, input.ViewDirV, input.LightDirV);
}
